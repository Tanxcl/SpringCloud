package com.orangecheng.leetcode.algorithm;

/**
 * @ClassName: _230131_2319
 * @description: 判断矩阵是否是一个 X 矩阵
 * @author: OrangeCheng
 * @create: 2023-01-31 15:08
 **/
public class _230131_2319 {

    /**
     * 如果一个正方形矩阵满足下述 全部 条件，则称之为一个 X 矩阵 ：
     *
     * 矩阵对角线上的所有元素都 不是 0
     * 矩阵中所有其他元素都是 0
     *
     * 给你一个大小为 n x n 的二维整数数组 grid ，表示一个正方形矩阵。
     * 如果 grid 是一个 X 矩阵 ，返回 true ；否则，返回 false 。
     *
     * 示例一:
     * 输入：grid = [[2,0,0,1],[0,3,1,0],[0,5,2,0],[4,0,0,2]]
     * 输出：true
     * 解释：矩阵如上图所示。
     * X 矩阵应该满足：绿色元素（对角线上）都不是 0 ，红色元素都是 0 。
     * 因此，grid 是一个 X 矩阵。
     *
     * 示例二:
     * 输入：grid = [[5,7,0],[0,3,1],[0,5,0]]
     * 输出：false
     * 解释：矩阵如上图所示。
     * X 矩阵应该满足：绿色元素（对角线上）都不是 0 ，红色元素都是 0 。
     * 因此，grid 不是一个 X 矩阵。
     */
    public static void main(String[] args) {
        _230131_2319 v = new _230131_2319();
        int[][] grid = {{1,0,0},{0,3,0},{1,0,1}};
        boolean b = v.checkXMatrix(grid);
        System.out.println(b);
    }

    public boolean checkXMatrix(int[][] grid) {
        for (int i = 0; i < grid.length; i++) {
            if (!(grid[i][i] > 0)){
                return false;
            }
            if (!(grid[i][grid[i].length-i-1] >0)){
                return false;
            }
            for (int j = 0; j < grid[i].length; j++) {
                if (i!=j && i+j!=grid[i].length-1){
                    if (grid[i][j] == 0){
                        continue;
                    }else{
                        return false;
                    }
                }
            }
        }
        return true;
    }
}
